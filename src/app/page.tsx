...
export default function Home() {
  ...
  useEffect(() => {
    ...
    trackPageView(session, "Homepage");
  ...
  const handleSignOutClick = () => {
    ...
    const sessionStart = session?.user?.loginTime;
  ...
  const currentUserId = generateOrGetUserId(session);
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <main className="flex flex-col items-center justify-center min-h-screen gap-8 p-8">
        <div className="text-center mb-8">
          ...
          {session ? (
            <div className="bg-green-900 border border-green-400 text-green-100 px-4 py-3 rounded mb-4">
              <p className="font-medium">Welcome back, {session.user?.name || session.user?.email}!</p>
              <p className="text-sm">You are now signed in and your interactions will be tracked with your account.</p>
            ...
            <div className="bg-yellow-900 border border-yellow-400 text-yellow-100 px-4 py-3 rounded mb-4">
              ...
              <p className="text-sm">Sign in to get personalized tracking and analytics.</p>
        ...
        <div className="mt-8 text-center text-sm text-gray-400">
          <p>All interactions are tracked with your user data in Google Analytics</p>
          <div className="mt-2 text-xs">
            ...
            {session && (
              <>
                ...
                <p>Login Time: {session.user?.loginTime}</p>
              </>
            )}
          </div>
        </div>
